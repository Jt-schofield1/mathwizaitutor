  ...
  try {
    ...
    if (process.env.GROQ_API_KEY && process.env.GROQ_API_KEY !== 'demo_key') {
      try {
        ...
        const hintLevelInfo = hintLevel === 1
        ...
        const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: `You are an expert math tutor for a ${gradeLevel}th grade student. Your job is to help them discover the answer through guided questions and strategic hints.
...
Provide educational hint #${hintLevel} that helps them learn:`
              }
            ],
            temperature: 0.8,
            max_tokens: 200,
    ...
    const hint = fallbackHints[Math.min(hintLevel - 1, fallbackHints.length - 1)];
